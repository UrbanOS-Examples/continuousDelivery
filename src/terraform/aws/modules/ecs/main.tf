...
data "template_file" "container_definitions" {
  template = "${file("${path.module}/templates/container_def.json")}"

  vars {
    cpu_units = "${var.ecs_task_def_cpu_units}"
    docker_image = "${var.ecs_task_def_docker_image}"
    mem_units = "${var.ecs_task_def_memory_units}"
    mem_res = "${var.ecs_task_def_memory_reservation}"
    task_def_name = "${var.ecs_task_def_name}"
  }
}
...
# Resource task definition for ECS
...
resource "aws_ecs_service" "ecs" {
  ...
  desired_count   = "${var.ecs_service_tasks_desired_count}"
#  iam_role        = "${aws_iam_role.ecs_cluster.arn}"
  depends_on      = ["aws_iam_role_policy.ecs_cluster"]
